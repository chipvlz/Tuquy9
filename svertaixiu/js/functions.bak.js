jQuery(function ($) {
    ...
    var userId = $('input[name="tx_token"]').val(); //+ Math.floor((Math.random() * 10000) + 1);
	...
	chenthoigian(-9999);
	try{
		socket_taixiu = io.connect('http://103.15.50.106:9094');//('http://115.84.183.203:9094');
	...
	//Fix load ajax
	$('body').on('click', '.tx_bet input, .numeric', function(){
    	$('body .tx_bet input, body .numeric').numeric();
    });
	
	$('body').on('keypress', '.tx_bet input', function (event) {//change focusout
        var n = $('body .tx_bet input').attr('name');
	...
	$('body').on('focus', '.tx_bet input', function () {//change focusout
	...
	$('body').on('focusout', '.tx_bet input', function () {//change focusout
    ...
    $('body').on('click', '.tx_use-xu, .tx_use-gem', function () {
